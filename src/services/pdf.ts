...
//
// ✅ 원칙 준수 구조 (v1.127.2+):
//   [1순위] 로컬 에이전트 POST /api/generate-statement
//           → drive_mirror 의 거래명세서양식.xlsx 를 MS Excel COM으로 열고
//           → 셀 데이터 주입 후 ExportAsFixedFormat(PDF) → Base64 반환
//
//   [2순위 Fallback] 에이전트 오프라인 시:
//           → HTML 직접 렌더링 + html2canvas + jsPDF (원칙상 임시 허용)
//           → ⚠️ 사용자에게 명시적 오프라인 경고 Toast 표시 필수
//
// ❌ 금지: buildExactStatementHTML()을 1순위 경로로 직접 호출하는 행위
import html2canvas from 'html2canvas';
import { jsPDF }    from 'jspdf';
import ExcelJS      from 'exceljs';
import { calcServicePeriod } from './excel';

const AGENT_URL = 'http://127.0.0.1:5175';

// ──────────────────────────────────────────────────────────────────────────────
// 오프라인 경고 Toast 표시
// ──────────────────────────────────────────────────────────────────────────────
function showOfflineWarningToast(message?: string) {
  try {
    const msg = message || '에이전트 오프라인 — HTML 임시 렌더링 모드로 작동 중입니다.';
    ...
    toast.style.cssText = [
      ...
      'box-shadow:0 4px 16px rgba(0,0,0,0.18)', 'white-space:pre-wrap',
      'line-height:1.5'
    ].join(';');
...
// ──────────────────────────────────────────────────────────────────────────────
// 에이전트 거래명세서 PDF 요청 (1순위)
// ──────────────────────────────────────────────────────────────────────────────
async function callAgentGenerateStatement(
  billing: any, details: any[], customer: any,
  contract: any, site: any, salesperson: any
): Promise<{ base64: string; filename: string } | null> {
  try {
    const res = await fetch(`${AGENT_URL}/api/generate-statement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ billing, details, customer, contract, site, salesperson }),
    ...
    if (data.success && data.base64Content) {
      ...
      return {
        base64: data.base64Content,
        filename: data.fileName || `거래명세서_${custName}_${siteName}_${ym}.pdf`
      };
    }
    return null;
  } catch {
...
// ──────────────────────────────────────────────────────────────────────────────
// [FALLBACK 전용] HTML 직접 렌더링 — 에이전트 오프라인 시에만 사용
// ⚠️ 반드시 showOfflineWarningToast() 와 함께 호출해야 함
// ──────────────────────────────────────────────────────────────────────────────
...
<meta charset="utf-8">
<style>
  html, body { margin:0; padding:0; width:700px; background:#fff;
    font-family:'Malgun Gothic','맑은 고딕',Arial,sans-serif; font-size:8.5px; color:#000; }
  * { box-sizing:border-box; }
  ...
  .V { color:#000; padding-left:3px; }
  .VC { color:#000; text-align:center; }
</style>
</head>
<body>
<div style="width:700px;padding:10px 0;background:#fff;position:relative;">
  <div style="position:absolute;top:10px;left:0;font-size:8.5px;color:#1B65A6;font-weight:bold;">청구번호: ${esc(billingNoStr)}</div>
  ...
  <div style="border:${outer};margin-bottom:6px;">
    <table>
      <colgroup><col style="width:20px"><col style="width:65px"><col style="width:120px"><col style="width:35px"><col style="width:108px"><col style="width:4px"><col style="width:20px"><col style="width:65px"><col style="width:120px"><col style="width:35px"><col style="width:108px"></colgroup>
    ...
    <table style="border-top:${solid}">
      <colgroup><col style="width:85px"><col style="width:261px"><col style="width:4px"><col style="width:85px"><col style="width:261px"></colgroup>
  ...
  <div style="border:${outer}">
    <table>
      <colgroup><col style="width:24px"><col style="width:18px"><col style="width:18px"><col style="width:105px"><col style="width:55px"><col style="width:62px"><col style="width:125px"><col style="width:42px"><col style="width:28px"><col style="width:55px"><col style="width:60px"><col style="width:52px"><col style="width:56px"></colgroup>
    ...
    <table style="border-top:${solid}">
      <colgroup><col style="width:50px"><col style="width:15px"><col style="width:95px"><col style="width:50px"><col style="width:15px"><col style="width:85px"><col style="width:40px"><col style="width:15px"><col style="width:95px"><col style="width:130px"><col style="width:110px"></colgroup>
...
// ──────────────────────────────────────────────────────────────────────────────
// [FALLBACK 전용] jsPDF 생성 내부 함수
// ──────────────────────────────────────────────────────────────────────────────
async function generateStatementJsPDF_Fallback(
  billing: any, details: any[], customer: any,
  contract: any, site: any, salesperson: any,
  templateUrl?: string, isCompressForEmail: boolean = false
  ...
  iframe.style.cssText = 'position:absolute;top:-9999px;left:0;width:700px;height:950px;border:none;visibility:hidden;';
  ...
  try {
    ...
    iBody.style.width = '700px'; iBody.style.overflow = 'hidden';
    const canvasScale = isCompressForEmail ? 1.6 : 2.0;
    const canvas = await html2canvas(iBody, {
      scale: canvasScale, useCORS: true, allowTaint: true,
      logging: false, backgroundColor: '#ffffff', width: 700, windowWidth: 700,
    });
    ...
    const ym       = billing?.billingYm || '';
    return { pdf, fileName: `거래명세서_${custName}_${sName}_${ym}.pdf` };
  } finally {
...
// ──────────────────────────────────────────────────────────────────────────────
// 브라우저 PDF 다운로드 메인 (에이전트 1순위 → HTML Fallback)
// ──────────────────────────────────────────────────────────────────────────────
export const downloadTransactionStatementPDF = async (
  billing: any, details: any[], customer: any, contract: any,
  site: any, salesperson?: any, templateUrl?: string, customFileName?: string
): Promise<void> => {
  // [1순위] 로컬 에이전트 — 정품 Excel COM PDF
  const agentResult = await callAgentGenerateStatement(billing, details, customer, contract, site, salesperson || {});
  if (agentResult) {
    const binaryStr = atob(agentResult.base64);
    ...
    link.href = url;
    link.download = customFileName || agentResult.filename;
    document.body.appendChild(link); link.click(); document.body.removeChild(link);
    URL.revokeObjectURL(url);
    return;
  }
  // [2순위 Fallback] 에이전트 오프라인 — 경고 Toast + HTML 임시 렌더링
  showOfflineWarningToast(
    '에이전트 오프라인 — HTML 임시 렌더링 모드로 거래명세서를 생성합니다.\n' +
    '원본 Excel 서식이 완벽히 보존되지 않을 수 있습니다.'
  );
  const { pdf, fileName } = await generateStatementJsPDF_Fallback(
...
// ──────────────────────────────────────────────────────────────────────────────
// 메일 첨부용 Base64 생성 (에이전트 1순위 → HTML Fallback)
// ──────────────────────────────────────────────────────────────────────────────
export const generateTransactionStatementPdfBase64 = async (
  billing: any, details: any[], customer: any, contract: any,
  site: any, salesperson?: any, templateUrl?: string
): Promise<{ filename: string; base64: string }> => {
  // [1순위] 로컬 에이전트 — 정품 Excel COM PDF
  const agentResult = await callAgentGenerateStatement(billing, details, customer, contract, site, salesperson || {});
  if (agentResult) {
    return { filename: agentResult.filename, base64: agentResult.base64 };
  }
  // [2순위 Fallback] 에이전트 오프라인 — 경고 Toast + HTML 임시 렌더링
  showOfflineWarningToast('에이전트 오프라인 — HTML 임시 렌더링 모드로 첨부 PDF를 생성합니다.');
  const { pdf, fileName } = await generateStatementJsPDF_Fallback(
    billing, details, customer, contract, site, salesperson, templateUrl, true
  );
