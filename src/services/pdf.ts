...
// 컨테이너 380px, onclone body 폭 강제고정, printW=95mm(현재 절반) → 오른쪽 자름 완전 해결
 ...
 * 거래명세서 HTML 생성
 * ─────────────────────────────────────────────────
 * 컨테이너: 380px  (border 2+2=4 → 내부 376px)
 * 정보 테이블: 각 섹션 186px | 분리대 4px | 186px = 376px (1:1)
 *   각 섹션: 11+32+74+21+48 = 186px
 * 날짜/입금: 38+124+38+176 = 376px
 * 품목:      14+9+9+148+20+39+46+42+49 = 376px
 * 합계:      21+8+77+21+8+56+17+8+56+70+34 = 376px
 * ─────────────────────────────────────────────────
 */
function buildStatementHTML(
    ...
    if (d) {
      ...
      itemRows += `
        <tr>
          ...
          <td style="border-right:${dotted};border-bottom:${dotted};padding-left:3px;overflow:hidden;text-overflow:ellipsis;">${escHtml(d.itemName)}</td>
          ...
          <td style="border-bottom:${dotted};padding-left:3px;overflow:hidden;text-overflow:ellipsis;">${escHtml(siteName)}</td>
        </tr>`;
    } else {
      itemRows += `
        <tr>
  ...
  const stampImg = stampDataUrl
    ? `<img src="${stampDataUrl}" style="position:absolute;right:2px;top:-5px;width:28px;height:28px;opacity:0.9;z-index:99;" />`
    : '';
...
<meta charset="utf-8">
<style>
  html, body { width:380px !important; max-width:380px !important; overflow:hidden !important; margin:0; padding:0; }
  * { box-sizing:border-box; }
  ...
  td, th { padding:0 2px; vertical-align:middle; overflow:hidden; white-space:nowrap; height:13px; max-width:100%; }
  .lbl { color:${blue}; font-weight:bold; text-align:center; font-size:7pt; }
  .val { color:#000; padding-left:3px; font-size:7pt; }
</style>
</head>
<body>
<div style="width:380px;max-width:380px;overflow:hidden;padding:4px 0;background:#fff;">

  <!-- 제목 -->
  <div style="text-align:center;margin-bottom:2px;">
    <span style="font-size:13pt;font-weight:bold;color:${blue};letter-spacing:5px;border-bottom:${dbl};padding-bottom:1px;display:inline-block;">거 래 명 세 표</span>
  </div>
  <div style="text-align:center;font-size:7pt;color:${blue};margin-bottom:4px;">(공급받는자 보관용)</div>

  <!-- 공급자 / 공급받는자 (내부 376px = 186+4+186) -->
  <div style="border:${outer};margin-bottom:3px;overflow:hidden;">
    <table style="width:376px;">
      ...
      <tr>
        ...
        <td class="val" style="border-right:${dotted};border-bottom:${dotted};font-size:5.5pt;">주식회사 기연리프트</td>
        ...
        <td class="lbl" style="border-right:${dotted};border-bottom:${dotted};font-size:6.5pt;">상호</td>
        <td class="val" style="border-right:${dotted};border-bottom:${dotted};font-size:5.5pt;">${escHtml(customer?.name || '')}</td>
        <td class="lbl" style="border-right:${dotted};border-bottom:${dotted};font-size:6.5pt;">대표</td>
      ...
      <tr>
        ...
        <td colspan="3" class="val" style="border-bottom:${dotted};font-size:5pt;">경기도 용인시 처인구 모현읍 갈담로112번길 21-3</td>
        <td class="lbl" style="border-right:${dotted};border-bottom:${dotted};font-size:6.5pt;">주소</td>
        <td colspan="3" class="val" style="border-bottom:${dotted};font-size:5pt;">${escHtml(customer?.address || '')}</td>
      </tr>
      <tr>
        <td class="lbl" style="border-right:${dotted};border-bottom:${dotted};font-size:6.5pt;">업태</td>
        <td class="val" style="border-right:${dotted};border-bottom:${dotted};font-size:5pt;">사업지원 및 임대서비스업 외</td>
        <td class="lbl" style="border-right:${dotted};border-bottom:${dotted};font-size:6.5pt;">종목</td>
        <td class="val" style="border-bottom:${dotted};font-size:5pt;">고소장비임대업 외</td>
        <td class="lbl" style="border-right:${dotted};border-bottom:${dotted};font-size:6.5pt;">업태</td>
  ...
  <!-- 날짜/입금 (376px = 38+124+38+176) -->
  <div style="border:${outer};margin-bottom:3px;overflow:hidden;">
    <table style="width:376px;">
      <colgroup>
        <col style="width:38px;"><col style="width:124px;">
        <col style="width:38px;"><col style="width:176px;">
      </colgroup>
      <tr>
        ...
        <td class="val" style="padding-left:4px;font-weight:bold;font-size:5.5pt;">신한은행 140-010-007060 , 주식회사 기연리프트</td>
      </tr>
    </table>
  </div>

  <!-- 품목 테이블 (376px = 14+9+9+148+20+39+46+42+49) -->
  <div style="border:${outer};overflow:hidden;">
    <table style="width:376px;">
      <colgroup>
        <col style="width:14px;"><col style="width:9px;"><col style="width:9px;">
        <col style="width:148px;"><col style="width:20px;">
        <col style="width:39px;"><col style="width:46px;">
        <col style="width:42px;"><col style="width:49px;">
      </colgroup>
    ...
    <!-- 합계 (376px = 21+8+77+21+8+56+17+8+56+70+34) -->
    <table style="border-top:${solid};width:376px;">
      <colgroup>
        <col style="width:21px;"><col style="width:8px;"><col style="width:77px;">
        <col style="width:21px;"><col style="width:8px;"><col style="width:56px;">
        <col style="width:17px;"><col style="width:8px;"><col style="width:56px;">
        <col style="width:70px;"><col style="width:34px;">
      </colgroup>
...
/**
 * 거래명세서 PDF 직접 다운로드
 * onclone으로 body 폭 380px 강제 고정 → printW=95mm(절반 크기) → A4 중앙 배치
 */
export const downloadTransactionStatementPDF = async (
  ...
  // 컨테이너 380px, overflow:hidden
  const container = document.createElement('div');
  container.style.cssText =
    'position:absolute;top:-9999px;left:-9999px;width:380px;max-width:380px;overflow:hidden;background:#fff;';
  ...
  try {
    ...
    const canvas = await html2canvas(target, {
      scale: 4,          // 고해상도 (380×4=1520px → 95mm = 400DPI)
      useCORS: true,
      logging: false,
      backgroundColor: '#ffffff',
      width: 380,
      height: target.scrollHeight,
      windowWidth: 380,  // 뷰포트 폭 강제 380px
      windowHeight: 2400,
      // onclone: body 폭을 380px로 강제 고정 (가장 강력한 제약)
      onclone: (_doc: Document, el: HTMLElement) => {
        const clonedDoc = el.ownerDocument;
        if (clonedDoc) {
          clonedDoc.documentElement.style.cssText = 'width:380px!important;overflow:hidden!important;';
          clonedDoc.body.style.cssText = 'width:380px!important;max-width:380px!important;overflow:hidden!important;margin:0;padding:0;';
        }
      },
    });
    ...
    // 절반 크기: printW = 95mm, A4 중앙 배치 (좌우 여백 약 57mm)
    const printW = 95;
    const mx = (pageW - printW) / 2;  // (210 - 95) / 2 = 57.5mm
    const my = 15;
