    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', fontSize: '13px' }}>
            ...
            <>
                ...
                <div style={{
                  ...
                  {INSPECTION_SPECS.map((spec, idx) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={!!checkedItems[spec.id]}
                        disabled={!canEdit || selectedItem.status === 'COMPLETED'}
                        onChange={e => setCheckedItems({ ...checkedItems, [spec.id]: e.target.checked })}
                      />
                      <span style={{ color: checkedItems[spec.id] ? 'var(--text-main)' : 'var(--text-muted)' }}>
                        {idx + 1}. {spec.label}
                      </span>
                    </label>
                  ))}
              ...
              <div>
                ...
                <textarea
                  ...
                  disabled={!canEdit || selectedItem.status === 'COMPLETED'}
                  placeholder="정비 조치 사항이나 특이사항이 있으면 입력해주세요."
                  style={{ width: '100%', fontSize: '12px', padding: '6px', borderRadius: '6px', border: '1px solid var(--border-color)' }}
                />
              </div>
      ...
      {showRejectModal && selectedItem && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="card" style={{ width: '100%', maxWidth: '420px', backgroundColor: 'var(--bg-card)' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '800', color: 'var(--danger)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <textarea
              ...
              style={{ width: '100%', fontSize: '12.5px', padding: '8px', borderRadius: '6px', border: '1px solid var(--border-color)', marginBottom: '16px' }}
              required
            />
