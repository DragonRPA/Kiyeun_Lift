...
> **문서 버전**: v1.1 (2026-09-01 21:55 기준)  
> **최종 적용 빌드**: v0.7.1.Build.31  
...
마이그레이션 시 자산 상태는 엑셀 `Col[8] 계약구분` 셀의 명시 값만을 기준으로 결정한다.  
**종료일 날짜는 자산 상태 판별에 사용하지 않는다.**

| 조건 | 계약 status | 자산 status | 현장 바인딩 |
|------|------------|------------|------------|
| Col[8] 값 없음, '연장', 기타 | `ACTIVE` | `RENTED` | ✅ 유지 |
| **Col[8]='연장' + 종료일 경과** | **`ACTIVE`** | **`RENTED`** | **✅ 유지 (연장/반납 미결)** |
| Col[8]='종료' 명시 (자사 자산) | `COMPLETED` | `AVAILABLE` | ❌ 해제 |
| Col[8]='종료' 명시 (전대 자산) | `COMPLETED` | `RENTED_RETURNED` | ❌ 해제 |

> **핵심 원칙**: 계약 종료일이 지났더라도 현장에서 연장할지 반납할지 결정되지 않은 상태이므로,  
> 엑셀에 **'종료'로 명시된 경우에만** COMPLETED/AVAILABLE로 처리한다.  
> 날짜 기반 자동 완료 처리는 **영구 금지**.

```typescript
// ✅ 올바른 판별 (Build.31 이후)
const isCompleted = contractStatusStr === '종료';   // Col[8] 명시값만 사용

// ❌ 금지된 판별 (Build.31 이전, 삭제됨)
// const isCompleted = contractStatusStr === '종료' || rowEndDate < '2026-08-01';
```
...
3단계: 엑셀 파일 선택 (드래그앤드롭 또는 파일 선택 버튼)
4단계: [과거 소급 청구서 생성] 체크박스 설정
       - 기본: 미선택 → 소급 청구서 미생성
       - 필요 시: 체크 후 시작 월 ~ 종료 월 지정 (예: 2026-01 ~ 2026-07)
       - ⚠️ 종료 월은 반드시 2026-07 이하 설정 (2026-08 당월 청구와 중복 방지)
5단계: 엑셀 파일 선택 버튼 클릭 → 파싱 완료 토스트에서 소급청구 N건 확인
6단계: [전체 데이터 일괄 적재 시작] 버튼 클릭
7단계: 진행 상황 로그 모니터링 (실시간 단계별 메시지 출력)
8단계: 완료 후 대사 리포트(ReconciliationReport) 6개 항목 확인
       + 고객 관리, 자산 관리, 계약 관리 화면에서 데이터 정상 여부 육안 검증
```

---

## 13. 알려진 이슈 및 해결 이력 (Build.22~31)
