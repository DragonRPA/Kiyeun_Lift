...
- **구글 서비스 계정 및 드라이브 저장소 설정 화면 신설**: 최고관리자(ADMIN) 전용의 환경 설정 화면을 신설하여 구글 OAuth 계정 이메일, 패스워드, Gmail API SMTP용 앱 비밀번호, 업무 유형별 구글 드라이브 보존 폴더명(렌탈계약서, 소모품납품증빙, 출고의뢰_증빙, 정비보고서_증빙)을 유연하게 편집하고 데이터베이스에 안전하게 기록하도록 구현했습니다.
