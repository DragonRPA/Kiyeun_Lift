...
- **소모품 및 자재 구매신청 & 구글 드라이브 연계 입고증빙 캡처 워크플로우 신설 (`Consumables.tsx` / `AppContext.tsx` / `db.ts` / `drive.ts`)**:
  ...
  - **구글 드라이브 입고 증빙 연계**: 도착한 소모품 입고 시(`REQ_INBOUND`), 구매완료된 미입고 물품을 선택한 후 **공급자 거래명세서 파일(PC 파일 경로 지정) 또는 핸드폰 촬영 사진**을 구글 드라이브의 **`소모품납품증빙`** 폴더에 업로드 완료해야만 최종 입고 확정이 가능한 강력한 입고 증빙 통제 로직을 완성했습니다. 이때 입고를 수행한 로그인 계정의 **입고처리자 이름**도 함께 저장되어 이력 관리에 반영되도록 보완했습니다.
