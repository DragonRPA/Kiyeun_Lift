...
## 📋 [저수준 브라우저 PDF 렌더러(`html2canvas`) 전면 폐기 및 의존성 제거]

### 🌟 반영 내용
- 브라우저 내장 HTML 렌더링 엔진(`html2canvas`, `jsPDF`)이 엑셀 고유의 서식을 지원하지 못해 저수준의 품질을 산출하는 문제를 원천 차단하기 위해 **관련 패키지와 모든 렌더링 코드를 프로젝트에서 완전히 삭제**했습니다.
- 이제 기연리프트 시스템에서 출력되는 모든 문서는 **품질 타협 없이 정품 엑셀(COM) 기반의 로컬 에이전트**를 통해서만 생성됩니다.

### 🗑️ 삭제 및 정리 내역
1. **패키지 삭제**: `html2canvas`, `jspdf` NPM 패키지 언인스톨 완료
2. **모듈 삭제**: `pdfBundle.ts`, `masterXlsxBundle.ts`, `pdf.ts` (거래명세서), `specSheetPdf.ts` (제원표) 전면 삭제
3. **UI 차단**:
   - `Products.tsx`: 제원표 PDF 브라우저 자동 생성 및 다운로드 버튼 무력화 (에러 얼럿 처리)
   - `Billings.tsx`: 거래명세서 PDF 다운로드 및 이메일 PDF 첨부 발송 기능 임시 무력화 (에러 얼럿 처리)
   - `GoogleConfig.tsx`: 샘플 서류팩 자동 생성 버튼 삭제
4. **향후 계획**: 차후 거래명세서 및 제원표도 단일 마스터 엑셀 기반 로컬 에이전트 연동으로 개편하여 100% 고품질 출력 지원 예정

---

# Release Notes (v1.131.0.Build.293 - 2026-08-30 11:47)

## 📋 [PDF 품질 저하(화면 깨짐) 해결 — 단일 마스터 엑셀 COM 엔진으로 전면 개편]

### 🌟 반영 내용
- 브라우저 기반의 HTML 변환 로직(`html2canvas`)이 엑셀 고유의 레이아웃(셀 병합, 테두리, 텍스트 줄바꿈 등)을 재현하지 못해 PDF 품질이 심각하게 파괴되던 문제를 해결했습니다.
- 로컬 에이전트(`agent.js`)의 정품 엑셀(COM) 엔진 로직을 **단일 마스터 엑셀 파일 처리 방식**으로 전면 재설계했습니다.

### 🚀 상세 개편 로직 (agent.js)
1. **단 1번의 엑셀 열기**: 기존에 3개의 엑셀 파일(계약서, 체크리스트, 점검결과서)을 각각 열고 닫느라 발생했던 심각한 속도 저하를 없애고, `01.계약서패키지_마스터.xlsx` 단 1개의 파일만 엽니다.
2. **시트 동적 복제 (Copy)**: 렌탈 장비 대수(N)만큼 `반입전체크리스트`와 `안전점검결과서` 시트를 내부적으로 복사하여 각각의 장비 제원/시리얼을 주입합니다.
3. **단 1번의 PDF Export**: 모든 데이터가 채워지면 워크북 전체를 `ExportAsFixedFormat` 1회 호출로 즉시 PDF 변환합니다.

### 💡 기대 효과
- **품질 완벽 복구**: 100% 정품 엑셀 렌더링 엔진을 사용하므로 엑셀 양식의 모든 시각적 요소가 단 1px의 오차도 없이 완벽하게 출력됩니다.
- **초고속 렌더링**: 엑셀 I/O 병목이 제거되어 기존 에이전트 로직 대비 체감 속도가 3배 이상 단축되었습니다.
- **브라우저 폴백 전면 폐기**: 품질이 떨어지는 브라우저 엔진(`generateContractFullDocumentBundlePdf`)을 모달에서 제거하고, 에이전트 미연결 시 즉각적인 경고창을 띄우도록 단일화했습니다.
...
#### 1. 출고 지시 완료 시 정식 계약번호 자동 바인딩 트리거 (`AppContext.tsx`, `smart_dispatch.tsx`)
- **생성 전/후 상태 분리**: 출고 지시 전에는 `(출고 요청 저장 시 자동 채번)`으로 표시되고, `[스마트 출고 요청 생성]` 실행 완료 즉시 DB에서 부여된 실제 정식 계약번호(예: `CT-20260829-001`)가 자동으로 바인딩되어 완성된 출고의뢰서가 형성됨.

#### 2. 청구담당자 완전 제거 및 4컬럼 50%:50% 완벽 대칭 레이아웃 (`smart_dispatch.tsx`)
- **청구담당자 제거**: 현장 검수원에게 불필요한 청구담당자 정보를 완전히 삭제하고, **영업담당자 (50%) | 현장담당자 (50%)** 1줄로 좌우 완벽한 균형을 확보.
- **4컬럼 고정 비율(`16% : 34% : 16% : 34%`)**: 1항, 2항, 3항 테이블에 `<colgroup>`을 지정하여 어떤 화면과 해상도에서도 좌우 여백이 휑하게 벌어지거나 우측으로 삐져나가는 현상을 100% 원천 차단.

#### 3. 좌측 상단 출력일시(`yyyy.MM.dd HH:mm:ss`) 및 우측 상단 '출고 완료자' 날인 결재란 탑재 (`smart_dispatch.tsx`)
- **출력일시 포맷 고도화**: 단순 '발행일' 대신 좌측 상단 계약번호 하단에 초 단위(`2026.08.29 21:05:30`) 출력일시 배치.
- **출고 완료자 날인란**: 우측 상단에 담당자의 책임성을 강화하는 **출고 완료자 서명/날인 박스**를 상시 렌더링.

---

# Release Notes (v1.130.0.Build.285 - 2026-08-29 20:52)

## 📋 [출고의뢰서 현장 직무 맞춤형 양식 전면 개편 및 여백 최적화 완비]

### 🌟 반영 내용

#### 1. 현장 검수/정비 직무 관점의 불필요 정보 전면 제거 (`smart_dispatch.tsx`)
- **불필요 필드 제거**: 출고정비 및 품질검수 담당자에게 무의미한 **담당자 메일주소**, **거래명세서 수신처**, **회계 정산 정보(마감일/결제일)**를 출고의뢰서 인쇄 양식에서 전면 삭제.
- **핵심 정보 추가**:
  - **계약번호**: 문서 좌측 상단 모서리에 `계약번호: CT-...` 형태로 상시 명확히 표출.
  - **영업담당자 정보**: `영업담당자: 홍길동 (010-1234-5678)`을 현장/청구담당자와 함께 명시하여 현장 정비 시 즉각적인 소통 채널 확보.
- **과잉 여백 및 박스 테두리 최적화**: 불필요한 이중 박스와 휑한 여백을 정돈하고, 정보 밀도가 높은 슬림 테이블 레이아웃을 적용하여 A4 1장에 매우 정갈하고 단정하게 안착.

---

# Release Notes (v1.130.0.Build.284 - 2026-08-29 19:16)

## 📋 [출고의뢰서 프리뷰 테이블 우측 오버플로우 넘침 버그 수정 완비]

### 🌟 반영 내용

#### 1. 프리뷰 테이블 고정 레이아웃(`table-layout: fixed`) 및 자동 줄바꿈 적용 (`smart_dispatch.tsx`)
- **버그 원인 해결**: 긴 이메일 주소, 긴 현장 주소 등으로 인해 브라우저 기본 테이블 렌더러가 용지 폭(800px)을 초과하여 우측으로 삐져나가던 오버플로우 현상 해결.
- **용지 100% 폭 칼맞춤**: 모든 테이블에 `tableLayout: 'fixed'` 및 `wordBreak: 'break-all'`을 적용하여, 아무리 긴 텍스트가 입력되더라도 정확히 백색 용지 테두리 안에 단정하게 줄바꿈되어 넘침이 1px도 발생하지 않도록 완결.

---

# Release Notes (v1.130.0.Build.283 - 2026-08-29 19:11)

## 📋 [출고의뢰서 프리뷰 영역 다크모드 색상 격리 및 글자 가독성 완비]

### 🌟 반영 내용

#### 1. 출고의뢰서 프리뷰 컨테이너 다크모드 전역 스타일 오염 차단 (`smart_dispatch.tsx`)
- **버그 원인 해결**: 다크모드 시 브라우저 전역 텍스트 색상(`var(--text-primary)` 등 연한 흰색/회색)이 프리뷰 백색 용지 내부의 `th`, `td`로 전파되어 글자가 보이지 않던 현상 해결.
- **명시적 다크모드 색상 격리**: 모든 테이블 헤더(`backgroundColor: #f8fafc`, `color: #334155`), 셀(`backgroundColor: #ffffff`, `color: #111827`), 테두리(`borderColor: #cbd5e1`)를 명시하여 다크모드/라이트모드 상관없이 항상 선명한 흑백 정품 양식으로 렌더링되도록 완결.

---

# Release Notes (v1.130.0.Build.282 - 2026-08-29 19:03)

## 📋 [출고의뢰서 인쇄 양식 미적용 항목 전면 제거 및 실제 요구 스펙만 딱 표출 개편]

### 🌟 반영 내용

#### 1. 출고의뢰서 인쇄 체크리스트 최적화 (`smart_dispatch.tsx`)
- **미적용 항목 전면 제거**: 과거 21개 전체 항목을 `(미적용)`으로 줄줄이 나열하던 방식을 폐지하고, 해당 출고 건에서 **실제로 요구/선택된 스펙 항목만 번호 순서대로 깔끔하게 표출** (`☑ 1. 철망 / 함석 설치`, `☑ 2. 상단 감지봉...` 등).
- **종이 출력 공간 극대화**: 불필요한 나열을 없애 인쇄 양식의 가독성과 정보 밀도를 대폭 향상.
- **21대 고정 수식어 폐지**: 고정된 21개 기준 문구를 제거하고 의뢰별 맞춤 스펙 동적 추출 방식으로 UI 정돈.

---

# Release Notes (v1.130.0.Build.281 - 2026-08-29 18:51)

## 📋 [출고의뢰서 브라우저 고품질 인쇄 모달 연동 및 A4 정품 양식 렌더링 완비]

### 🌟 반영 내용

#### 1. 브라우저 고품질 인쇄 모달 파이프라인 전환 (`smart_dispatch.tsx`)
- **실제 프린터 100% 인쇄 보장**: 윈도우 mshtml 무음 실패 위험을 제거하고, `[출고의뢰서 인쇄]` 클릭 시 브라우저 내장 인쇄 대화상자가 즉시 활성화되어 기본 프린터(`Apeos C2060`)로 바로 출력 가능.
- **A4 정품 인쇄 CSS 완비**: `@page { size: A4 portrait; margin: 12mm 15mm; }` 및 `-webkit-print-color-adjust: exact`를 적용하여 테이블 테두리, 배경 음영, 폰트가 A4 용지 1장에 완벽하게 꽉 차서 렌더링됨.
- **인쇄 후 팝업 자동 정리**: 인쇄 전송 완료 또는 취소 시 인쇄 창이 백그라운드에서 자동으로 닫히도록 라이프사이클 처리.

---

# Release Notes (v1.130.0.Build.280 - 2026-08-29 18:41)

## 📋 [출고 요청 메뉴 상단 로컬 프린터 지정 및 출고의뢰서 인쇄 툴바 탑재 완비]

### 🌟 반영 내용

#### 1. 출고 요청 메뉴 상단 로컬 프린터 제어 툴바 상시 배치 (`smart_dispatch.tsx`)
- **로컬 프린터 목록 조회 및 지정**: 로컬 에이전트(`agent.js`)와 실시간 연동되어 PC에 설치된 프린터 목록을 자동 수신하고, 담당자가 원하는 출력 프린터를 선택하면 브라우저에 영구 저장(`localStorage`).
- **상단 원터치 [출고의뢰서 인쇄] 연동**: 페이지 스크롤을 맨 아래로 내리지 않고도 상단 툴바에서 즉시 `[출고의뢰서 인쇄]`를 클릭하여 지정된 로컬 프린터로 0초 무팝업 다이렉트 출력 실행.
- **전사 표준 헌장 준수**: 레이블-입력창 세로 스택 구조(3.4) 및 무수식어 건조한 명사 단일 표준(3.1) 이행.

---

# Release Notes (v1.130.0.Build.279 - 2026-08-29 18:33)

## 📋 [출고검수 의뢰 카드 요구 스펙 태그 및 할당 장비 관리번호 분리 표출 완비]

### 🌟 반영 내용

#### 1. 출고 검수 의뢰 카드 정보 계층 분리 및 시인성 개선 (`outbound_inspections.tsx`)
- **버그 원인 해결**: '요구 스펙: 3개 항목' 라벨 우측에 할당된 장비 관리번호(`K10064`, `K10065`, `K10066`)가 렌더링되던 UI 혼선을 완전히 분리.
- **할당 장비 영역**: 포함 장비 박스 내에 `할당 장비: K10064, K10065...` 형태로 명확하게 표출.
- **요구 스펙 영역**: 실제 계약/출고 시 요구된 기술/안전 검수 스펙 항목 태그(`[철망 설치 검수]`, `[배터리 단자 마킹]`, `[부착물 세트]` 등)가 직관적인 배지 형태로 표출되도록 개편.

---

# Release Notes (v1.130.0.Build.278 - 2026-08-29 18:27)

## 📋 [출고검수 메뉴 기본 상태 필터 '접수 대기(PENDING)'로 변경 완비]

### 🌟 반영 내용

#### 1. 출고 검수 기본 뷰 최적화 (`outbound_inspections.tsx`)
- **실무자 ToDo 중심 뷰 적용**: 메뉴 진입 시 전체 의뢰 목록이 아닌 당장 처리해야 할 **'🟡 접수 대기 (PENDING)'** 목록이 기본으로 즉시 표출되도록 초기 필터 상태를 변경.

---

# Release Notes (v1.130.0.Build.277 - 2026-08-29 18:22)

## 📋 [장비할당 부분 수량 할당 완벽 지원 및 1대만 선택해도 할당 버튼 즉시 활성화]

### 🌟 반영 내용

#### 1. 부분 수량 할당(Partial Assignment) 완벽 지원 (`asset_assignment.tsx`)
- **요구 수량 대비 부분 할당 지원**: 2대(또는 $N$대) 요구 모델 선택 후 현장에 준비된 장비 1대(또는 $K$대)만 선택하더라도 `[선택 장비 할당 (1대)]` 버튼이 즉시 활성화되어 바로 할당 가능.
- **슬롯 순차 매핑 및 잔여 슬롯 유지**: 선택된 $K$대의 장비가 미할당 슬롯 앞쪽 $K$개에 안전하게 매핑되며, 남은 슬롯은 미할당 상태로 자동 보존되어 추가 할당 작업 가능.

---

# Release Notes (v1.130.0.Build.276 - 2026-08-29 17:42)

## 📋 [장비 할당 취소 이중 팝업 제거 및 단일 원자적 배치 완결 후 1회 알림 전환]

### 🌟 반영 내용

#### 1. 사전문답 confirm 제거 및 즉시 실행 ➔ 완료 시 1회 알림 표준화 (`asset_assignment.tsx`)
- **이중 팝업 제거**: 버튼 클릭 시 불필요한 사전 확인창을 거치지 않고 즉시 취소 작업을 실행하며, 전체 작업이 100% 완료된 시점에만 딱 **1회의 완료 알림(alert)**을 표출하도록 개편.

#### 2. 다중 장비 일괄 할당 취소 단일 원자적 배치 메소드 탑재 (`AppContext.tsx`)
- **루프 연쇄 호출 레이스 컨디션 해결**: `batchUnassignAssetsFromContract(caIds)`를 구현하여, $N$대의 장비 취소를 메모리상에서 일괄 처리한 후 원격 DB 쓰기 및 전역 리렌더링을 **단 1회만 완결 실행**.
- 마지막 1개가 처리되는 도중 팝업이 뜨는 타이밍 엇갈림 현상 원천 차단.
...
#### 1. 오버플로우 방지 기준 모델별 요구수량(Model Quota)으로 완전 전환 (`asset_assignment.tsx`)
- **모델별 요구수량 초과 원천 차단**: 계약 총수량이 아닌 **해당 모델의 요구 수량(예: GS-1930 2대)**을 기준으로 카드 클릭, 관리번호 빠른 입력, 자동 선택 전 경로에서 초과 선택(3대 이상)을 완벽히 차단 (`getModelQuotaForAsset` 적용).
- 타 모델 요구수량 침범 및 교차 오할당 위험 0% 실현.

#### 2. 계약 선택 시 첫 번째 미할당 모델 자동 단일 포커스 연동
- 출고 요청 건(계약 카드) 클릭 시 미할당된 첫 번째 모델의 슬롯들이 자동으로 단독 활성화되어 즉시 가용 장비가 필터링되고 해당 모델 수량만큼 작업할 수 있도록 UX 최적화.

---

# Release Notes (v1.130.0.Build.273 - 2026-08-29 17:27)

## 📋 [출고 검수 전 장비 할당 취소 및 임대가능(AVAILABLE) 상태 자동 원복 기능 완비]

### 🌟 반영 내용

#### 1. 장비 할당 취소 트랜잭션 메소드 구현 (`AppContext.tsx`)
- **안전한 상태 롤백 복원**: `unassignAssetFromContract(contractAssetId)`를 구현하여, 출고 전 단계에서 할당된 장비를 언제든지 취소 가능.
- **자산 상태 자동 원복**: 장비 상태를 즉시 **`AVAILABLE` (`임대가능`)**으로 복원하고, 계약 매핑 정보(고객사, 현장, 계약기간) 및 대기 중(`PENDING`)인 출고 검수 의뢰건을 깔끔히 자동 정리.

#### 2. 모델 그룹 카드 내 실시간 할당 취소 UI 탑재 (`asset_assignment.tsx`)
- 할당 완료된 장비 태그 배지 옆에 **`[✕ 취소]`** 버튼 제공 ➔ 원클릭 개별 할당 해제.
- 카드 상단에 **`[✕ 이 모델 할당 전체 취소]`** 버튼 제공 ➔ 해당 모델 장비 일괄 원복 지원.

---

# Release Notes (v1.130.0.Build.272 - 2026-08-29 17:26)

## 📋 [일괄 장비할당 원자적 단일 배치 트랜잭션 전환 및 모델명 정규화 그룹핑 완비]

### 🌟 반영 내용

#### 1. 일괄 할당 단일 원자적 배치 트랜잭션 탑재 (`AppContext.tsx`)
- **비동기 루프 레이스 컨디션 완벽 해결**: 기존 $N$회 연쇄 호출로 발생하던 중간 리렌더링 및 팝업 타이밍 엇갈림을 해결하기 위해 `batchAssignAssetsToContract(pairs)` 단일 배치 함수를 구현.
- 모든 슬롯 및 자산 할당을 메모리상에서 원자적으로 처리한 후, DB 동기화(`awaitPendingWrites`) 및 전역 리렌더링(`refreshAllData`)을 **단 1회만 완결 실행**.

#### 2. 모델명 정규화(Normalization) 그룹핑 및 계약 expectedModel 보존
- **동일 모델 분할 버그 해결**: `GS-2646`과 `GS2646` 등 하이픈/공백 차이로 인해 카드가 2줄로 갈라지던 현상을 `normalizeModelKey` 기반 100% 통합 그룹핑으로 완전 해결.
- 장비 할당 시 기존 계약의 고유 `expectedModel`을 덮어쓰지 않고 영구 보존하도록 수정.

---

# Release Notes (v1.130.0.Build.271 - 2026-08-29 17:03)

## 📋 [장비할당 좌측 모델명×수량 그룹 카드 전면 개편 및 선택 오버플로우 원천 차단 완비]

### 🌟 반영 내용

#### 1. 좌측 영역 모델명 × 수량 그룹 카드 뷰 전면 개편 (`asset_assignment.tsx`)
- **개별 6줄 리스트 나열 전면 제거**: 계약 내 모델별로 `GS-2646 × 4대 (미할당 4대 / 기할당 0대)`, `GS-1930 × 2대` 형태로 압축된 **모델 그룹 카드** 제공.
- 카드 내 `[모델명 N대 선택]` 원클릭 토글 버튼 제공 및 기할당된 장비 태그 배지 표출.
- 상단 `[미할당 전체 선택 (총 N대)]` 지원.

#### 2. 가용 장비 선택 수량 상한(Max Limit) 제어 및 오버플로우 원천 차단
- **선택 수량 상한 제어**: 현재 선택된 슬롯 수(또는 전체 미할당 슬롯 수)를 초과하여 장비를 무제한으로 선택할 수 없도록 카드 클릭, 관리번호 빠른 입력, 자동 선택 전 경로에서 오버플로우 차단 및 안내 팝업 적용.

---

# Release Notes (v1.130.0.Build.270 - 2026-08-29 17:00)

## 📋 [출고검수 관리 정비사유 지정 시 해당 점수 부여 및 기본 벌점 5점 중복 제외 로직 완비]

### 🌟 반영 내용

#### 1. 관리 정비사유(마스터 항목) 지정 시 점수 단일 부과 로직 (`AppContext.tsx`)
- **전사 정책 반영**: 출고 검수 장비 교체 시 기존에 관리되고 있는 정비사유(예: 배터리 방전 +10점, 유압누유 +15점 등)를 지정한 경우, 해당 사유의 가중 점수만 가산하고 **일반 기본 벌점 5점은 중복 부여하지 않음** (`점수 = 기존점수 + 사유점수`).
- **미지정 시 기본 벌점 유지**: 관리 정비사유를 선택하지 않고 단순 교체한 경우에만 기본 벌점 +5점 가산.

#### 2. 출고검수 교체 모달 내 정비사유 마스터 셀렉터 및 실시간 점수 안내 탑재 (`outbound_inspections.tsx`)
- 관리 정비 사유 마스터(`inspectionChecklistItems`) 드롭다운 탑재.
- 선택된 사유에 따른 실시간 점수 가산 안내 배너 제공 (`[사유명] 지정됨 ➔ 정비점수 +N점 부여 (기본 5점 벌점 제외)`).

---

# Release Notes (v1.130.0.Build.269 - 2026-08-29 16:55)

## 📋 [출고검수 탈락 교체 시 정비점수 +5점 가산 및 신중한 할당 강제 방어 로직 완비]

### 🌟 반영 내용

#### 1. 출고 검수 장비 교체 시 정비점수(Maintenance Score) 무조건 +5점 가산 (`AppContext.tsx`)
- **전사 정책 반영**: 출고 검수 단계에서 탈락되어 교체되는 기존 장비(구장비)에 대해 사유 유무와 무관하게 `maintenanceScore`를 `(기존점수 + 5)`점으로 가산하여 DB 저장.
- **도입 목적**: 장비 할당의 신중함 강제 및 고의/과실/게으름으로 인한 사유 기록 누락 방어.

#### 2. 탈락 장비 이력/비고 무누락 영구 보존
- 사유 미입력 시에도 `[출고검수 교체(벌점+5, 총점:N점)] YYYY-MM-DD: 출고검수 탈락 교체(사유미기재)` 형태로 장비 비고/메모에 자동 기록.
- 출고검수 교체 모달 내 정책 안내 배너 표출 (`outbound_inspections.tsx`).

---

# Release Notes (v1.130.0.Build.268 - 2026-08-29 16:52)

## 📋 [장비할당 모델명×수량 그룹핑 표기, 다중 장비 멀티셀렉트 일괄할당 및 관리번호 빠른 입력 완비]

### 🌟 반영 내용

#### 1. 모델명 × 수량 요약 및 그룹핑 바 (`asset_assignment.tsx`)
- 출고 대기 계약 카드 및 상세 워크보드에 `JCPT1412AC × 11대 (미할당 11대)` 형태로 모델별 그룹핑 칩 및 요약 표출.
- 모델 그룹 클릭 시 해당 모델의 미할당 슬롯들을 1초 만에 원클릭 일괄 선택 지원.

#### 2. 다중 슬롯 / 다중 장비 멀티셀렉트(Multi-Select) 일괄 할당 파이프라인
- 슬롯 $N$개 선택 ➔ 우측 가용 장비 $N$대 다중 선택(체크박스/카드 클릭) ➔ `[선택 장비 일괄 할당 (N대)]` 버튼 1번 클릭으로 일괄 할당 완결.
- **[추천순 자동선택]** 버튼: 선택된 슬롯 수량($N$)만큼 정비 점수 최우선 가용 장비 $N$대를 원클릭 자동 체크.

#### 3. 관리번호 빠른 연속 입력 인터페이스 (실무자 요구사항 반영)
- 텍스트 입력창에 관리번호 전체 또는 뒷자리(예: `K10437, K10438, 10439`)를 쉼표나 공백, 엔터로 입력 시 가용 장비를 자동 탐색하여 선택 목록에 즉시 추가하는 키보드 스마트 인풋 완비.

---

# Release Notes (v1.130.0.Build.267 - 2026-08-29 16:47)

## 📋 [계약서 자산 1대당 1행 명시적 표기 및 자산수 초과 시 동적 페이지 확장 엔진 완비]

### 🌟 반영 내용

#### 1. 계약서 엑셀 변환 로직 자산 행 명시적 바인딩 (`agent.js`)
- **1줄당 자산 1대 완벽 표기**: `품목(모델명)`, `수량(1)`, `장비번호(S/N)`(윗줄: S/N, 아랫줄: 관리번호), `임대료(단가)`, `소계(단가)`를 자산 1대당 1행씩 명시적으로 기입.
- **자산 수 초과 시 자동 페이지 확장**: 12대 이하일 때는 A4 1페이지로 정밀 피팅(`FitToPagesTall = 1`), 13대 이상일 때는 엑셀 페이지가 자연스럽게 2페이지 이상으로 자동 확장(`FitToPagesTall = $false`)되도록 동적 인쇄 영역 설정.

---

# Release Notes (v1.130.0.Build.266 - 2026-08-29 16:35)

## 📋 [정품 엑셀 COM 기반 다중 자산 7종 계약 서류팩 엔진 및 제원표 엑셀 표준 템플릿 정립]

### 🌟 반영 내용

#### 1. 로컬 에이전트 다중 자산($N$대) 엑셀 COM 배치 엔진 전면 고도화 (`agent.js`)
- **`01.계약서.xlsx`**: 계약 정보, 현장 정보, 투입 자산 목록($N$대) 일괄 기입 후 1p 출력.
- **`02.반입전체크리스트.xlsx`**: 투입 자산 $N$대 루프 $\rightarrow$ 관리번호/모델명 치환 후 71개 정품 서식 $N$p 일괄 출력.
- **`03.안전점검결과서.xlsx`**: 투입 자산 $N$대 루프 $\rightarrow$ 제원/차량번호/인증일 치환 후 17개 법정 기준 $N$p 일괄 출력.
- **후반부 완벽 결합**: 투입 모델별 정규 문서(8p) + PL보험증권 2년 연속분(2p) + 사업자등록증(1p) + 통장사본(1p) 결합하여 37페이지 단일 PDF 완성.

#### 2. 제원표 기본 엑셀 템플릿 정립 (`04.제원표양식.xlsx`)
- A4 세로 인쇄 최적화 제원표 엑셀 기본 서식을 `C:\KiyeunAgent\drive_mirror\04.제원표양식.xlsx` 및 Cloudflare R2 버킷에 정립.
- **트리거 원칙 확립**: 제품 관리에서 스펙 저장 시에만 1회 PDF 생성 트리거 작동 ➔ 계약 관리에서는 기 작성된 제원표 PDF를 무연산 고속 병합.

#### 3. 웹 프론트엔드 모달 엑셀 에이전트 1순위 파이프라인 연동 (`ContractDocumentBundleModal.tsx`)
- 웹 화면에서 [7종 통합 서류팩 PDF / 이메일] 클릭 시 로컬 에이전트의 정품 엑셀 엔진을 직접 호출하여 무결점 37페이지 PDF 수신 및 미리보기/다운로드/이메일 발송 연동.

---

# Release Notes (v1.130.0.Build.265 - 2026-08-29 16:00)

## 📋 [로컬 에이전트 Gmail SMTP 엔진 탑재 및 7종 통합 계약 서류팩 실시간 이메일 발송 테스트 완료]

### 🌟 반영 내용

#### 1. 로컬 에이전트 이메일 발송 라우트 탑재 (`agent.js`)
- `POST /api/send-email`: 로컬 에이전트가 단독으로 Gmail SMTP (포트 465 SSL)을 통해 37페이지 첨부파일(PDF Base64)을 포함한 대용량 이메일을 직접 발송할 수 있도록 통신 라우트 내장.
- Cloudflare R2 원본 파일 256개 로컬 미러링 캐시(`C:\KiyeunAgent\drive_mirror\`) 동기화 완결.

#### 2. 이메일 서비스 이중 다중화(Dual-Path) 아키텍처 적용 (`email.ts`)
- 1순위 로컬 에이전트(`http://127.0.0.1:5175/api/send-email`) ➔ 미응답 시 2순위 Vercel 서버리스 API 자동 폴백(Fail-over) 구조 적용.

#### 3. 실서버 기능 작동 테스트 완료
- 대상 계약: 주식회사 세보엠이씨 — 평택삼성전자 P4 현장
- 수신처: `77.victor.lee@gmail.com`
- 첨부: `[기연리프트]_계약서류팩_주식회사세보엠이씨_평택삼성전자P4(37p).pdf` (2.7 MB, 37페이지)
- 결과: Message-ID `<843091db-8a2d-d494-d5a4-7634d614e271@gmail.com>` 정상 발송 확인 (HTTP 200 OK).
...
#### 1. 7단계 단일 PDF 번들 자동 조립 엔진 구현 (`pdfBundle.ts`)
- **1. 계약서**: A4 1페이지 실물 표준 서식 (계약처, 인도장소, 자산 목록, 운송료 청구 기준, 주의사항 완비).
- **2. 자산별 반입 전 CHECK LIST**: 계약 투입 자산 $N$대 각각에 대해 관리번호/모델명을 1:1 매핑한 71개 점검표 $N$장 동적 렌더링.
- **3. 자산별 안전점검 결과서**: 계약 투입 자산 $N$대 각각에 대해 차량번호/모델명/제조년도/장비중량/안전인증일을 주입한 안전점검표 $N$장 동적 렌더링.
- **4. 모델별 `Eq_doc/{모델명}/` 정규 서류 일체**: 투입된 $M$개 장비 모델의 Cloudflare R2 원본 PDF(제원표, 안전인증서, 작동법, 비상하강법 등) 각 1부씩 바이너리 직접 복사 병합.
- **5. 생산물배상책임(PL)보험증권**: 계약 시작일~종료일(또는 장기)을 분석하여 2025~2026년 증권 및 2026~2027년 갱신 증권을 1~2장 자동 선별 병합.
- **6·7. 사업자등록증 & 통장사본**: Cloudflare R2 원본 PDF(`09.사업자등록증.pdf`, `10.통장사본.pdf`) 바이너리 복사 병합.
- 첨부 12대 2개 모델 샘플 조건에서 **정확히 37페이지** 단일 PDF 무결점 병합 검증 완료.

#### 2. 계약 서류팩 모달 및 이메일 발송 연동 (`ContractDocumentBundleModal.tsx`, `Contracts.tsx`)
- 계약 목록 및 상세에서 `[7종 통합 서류팩 PDF / 이메일]` 버튼을 통해 7단계 실시간 조립 진행률 표시, [새 창 미리보기], [다운로드], [이메일 클라이언트 발송] 원스톱 지원.
- 전사 다크/라이트 테마 토큰 완벽 적용.

---

# Release Notes (v1.129.0.Build.262 - 2026-08-29 14:26)

## 📋 [기존 원본 PDF 보호 및 미보유 모델 대상 자동 제원표 생성 정밀 분기]

### 🌟 반영 내용

#### 1. 제품 저장 핸들러 제원표 자동 생성 조건부 정밀 분기 (`Products.tsx`)
- 기존에 이미 실물 원형 PDF(`specSheetUrl`)가 존재하는 모델은 저장(수정) 시 재생성 로직을 건너뛰어 **기존 원본 PDF의 1:1 보존을 100% 보장**.
- 기존 PDF가 없는 신규 등록/미보유 모델에 한해서만 백그라운드 자동 생성을 트리거하여 Cloudflare R2에 표준 제원표 PDF를 신규 저장하고 DB에 영구 등록.

---

# Release Notes (v1.129.0.Build.261 - 2026-08-29 13:54)

## 📋 [클라우드 문서함 모달 다크/라이트 테마 일관성 완전 복원]

### 🌟 반영 내용

#### 1. 클라우드 문서함 모달 전사 CSS 테마 변수 일원화 (`Products.tsx`)
- 임시 인라인 하드코딩 색상(`colorScheme: light`, `#ffffff`, `#111827` 등)을 전면 제거하고 전사 CSS 토큰(`var(--bg-card)`, `var(--bg-app)`, `var(--border-color)`, `var(--text-main)`, `var(--text-secondary)`, `var(--text-muted)`, `var(--primary)`) 기반으로 일괄 재정비.
- **다크 모드**: 어두운 모달 카드 배경 + 밝은 텍스트/헤더/테두리로 일관된 시인성 보장.
- **라이트 모드**: 밝은 모달 카드 배경 + 어두운 텍스트/헤더/테두리로 자연스러운 렌더링 유지.
...
#### 1. 청구 도래 미생성 계약 실시간 감지 알림 바 및 일괄 생성 엔진 (`AppContext.tsx`, `Billings.tsx`)
- 오늘 날짜(`todayDate`) 기준으로 살아있는 계약 중, 고객 요청 청구기준일(`billingDay`/`statementClosingDay`)이 도래/경과했거나 전월 미청구된 계약을 실시간 자동 감지 (`getDueContractsForBilling`).
- 상단에 `[📢 오늘 기준 청구 도래 미생성: N건]` 알림 바 및 `[도래 계약 기본 청구 일괄 생성]` 원터치 버튼 마운트.
- 헌장 4.1에 따른 자산별 정밀 일할 렌탈료 및 완료된 유료 AS 수리비 자동 합산, 보유 선수금(예치금) 상계 차감 반영하여 `REQUESTED` 결재대기 청구서 일괄 생성 (`generateDueBillings`).

#### 2. 청구서 개별 검토 · 승인 완료 · 취소 및 수정 재생성 시스템 (`Billings.tsx`, `AppContext.tsx`)
- 청구 목록 테이블 관리 컬럼에 `[검토]`, `[완료(승인)]`, `[취소/재생성]`, `[취소]` 인터랙션 탑재.
- `[취소/재생성]` 모달: 청구귀속월, 발행일자, 품목별 단가/수량/기간 수정 및 추가 항목(운송료, 수리비, 부대비용, 할인 등) 추가/삭제 지원.
- 수정사항 반영 시 기존 청구서는 감사 추적성을 위해 안전하게 취소(`REJECTED`) 마감하고, 수정된 신규 청구서(`REQUESTED`)로 즉시 재발행 (`regenerateBilling`).

#### 3. 정비 대장 및 정비사 차량 소모품 적재 재고 관리 체계 고도화 (`Consumables.tsx`, `Repairs.tsx`, `db.ts`)
- `MechanicConsumableStock` 정비사 차량별 적재 재고 모델 신설 및 본사 창고 ↔ 기사 차량 간 이동/반납 트랜잭션 구축.
- 출장 정비 라이프사이클(`SCHEDULED` -> `IN_PROGRESS` -> `COMPLETED`/`UNRESOLVED`) 및 미완료 사유/후속 조치 기록 체계 지원.
...
### 🌟 반영 내용
1. **임의의 웹 HTML 모방본 전면 폐지 및 정품 엑셀 직접 변환 엔진 탑재 (`agent/agent.js`)**:
   - `POST /api/generate-contract-bundle` 엔드포인트 신설.
   - 로컬 `C:\KiyeunAgent\drive_mirror\` 내 실제 엑셀 서식(`01.계약서.xlsx`, `02.반입전체크리스트.xlsx`, `03.안전점검결과서.xlsx`)을 열어 셀 태그(`{Today}`, `{고객명}`, `{대표자}`, `{현장명}` 등)를 100% 원본 서식 그대로 직접 치환.
   - Microsoft Excel 16.0 COM 자동화를 통해 공식 인쇄 영역 및 여백이 보존된 고해상도 벡터 PDF를 직접 출력.
2. **정품 6종 통합 서류팩 무손실 바이너리 병합 및 로컬 자동 아카이빙**:
   - 엑셀 변환 PDF 3장 + Cloudflare R2 원본 PDF 3장(`08.보험증권`, `09.사업자등록증`, `10.통장사본`)을 `pdf-lib`으로 무손실 단일 6페이지 PDF로 자동 결합.
   - 로컬 문서고(`C:\KiyeunAgent\문서고\YYYY-MM\...`)에 자동 영구 아카이빙 및 프런트엔드로 Base64 직접 전달.
3. **프런트엔드 연동 1순위 승격 (`src/services/pdfBundle.ts`)**:
   - 6종 서류팩 생성 시 로컬 에이전트의 정품 엑셀 COM 자동화를 1순위로 즉시 호출.
4. **[표준 헌장 8.3] 회사 성장 4대 요소 `후회 (REGRET)` skelton 기록 및 푸시 완결**:
   - `skelton/후회/2026-08_엑셀_정품_서식_직접수정_및_PDF_병합_원칙.md` 작성 및 원격 저장소 커밋·푸시 완료.

---

# Release Notes (v1.126.0.Build.243 - 2026-08-19 18:21)

## 🛡️ [에이전트 자립형 S3 SigV4 스캔 & 직접 무손실 다운로드 파이프라인 완결]

### 🌟 반영 내용
1. **에이전트 단독 기동 및 자가 동기화 엔진 결함 완벽 교정 (`agent/agent.js`)**:
   - SigV4 헤더 누락으로 인한 HTTP 400 차단 버그 원천 해결 (`x-amz-content-sha256` 정합).
   - 비정상 생성된 디렉토리 오염물 자동 감지 및 제거 후 파일 쓰기(`EISDIR` 예방).
   - 기동 시 즉시 1회 R2 버킷 스캔 & 1시간 주기 백그라운드 자가 점검 완결.
2. **동기화 파이프라인 에이전트 직결 구조화 (`src/services/driveMirrorSync.ts`)**:
   - `executeDriveMirrorSync` 실행 시 브라우저 메모리가 아닌 로컬 에이전트의 `POST /api/trigger-sync`를 1순위로 직접 가동하여 1~2초 만에 로컬 디스크로 무손실 직접 다운로드.
3. **에이전트 프로세스 핫 리로드 지원 (`C:\KiyeunAgent\start-agent.bat`)**:
   - 배치 파일 내 자동 재시작 루프 탑재.

---

# Release Notes (v1.125.0.Build.242 - 2026-08-19 17:58)

## ⚡ [새로고침 시 중복 동기화 트래픽 제거 및 에이전트 자립형 1시간 주기 동기화 최적화]

### 🌟 반영 내용
1. **프런트엔드 새로고침(F5) 부하 0% 경량화 (`src/components/AgentHeaderBadge.tsx`)**:
   - 브라우저 새로고침 시마다 발생하던 `executeDriveMirrorSync` 자동 호출 제거.
   - 에이전트 `GET /api/mirror-status`를 통해 0.001초 만에 로컬 파일 현황만 가볍게 조회하도록 최적화.
2. **에이전트 자립형 백그라운드 스캐너 고도화 (`agent/agent.js`)**:
   - 기동 시 1회 자가 동기화 + 이후 **1시간(3,600,000ms) 주기 백그라운드 자동 점검** 탑재.
   - `POST /api/trigger-sync` 엔드포인트 신설로 프런트엔드 수동 동기화 요청 시 에이전트 내부 SigV4 스캔 엔진 직접 구동.

---

# Release Notes (v1.124.0.Build.241 - 2026-08-19 17:50)

## 📑 [Cloudflare R2 6종 통합 계약 서류팩 PDF 생성 및 계약 DB 연동 테스트 기능 탑재]

### 🌟 반영 내용
1. **Cloudflare R2 6종 서류 무손실 병합 엔진 (`src/services/pdfBundle.ts`)**:
   - `01.계약서`, `02.반입전체크리스트`, `03.안전점검결과서` (선택된 계약 DB 데이터 자동 주입 렌더링).
   - `08.생산물배상책임보험증권.pdf`, `09.사업자등록증.pdf`, `10.통장사본.pdf` (Cloudflare R2 및 로컬 에이전트 캐시 원본 PDF 무손실 바이너리 병합).
   - 생성 즉시 로컬 에이전트 문서고(`C:\KiyeunAgent\문서고\YYYY-MM\...`) 자동 아카이빙 연동.
2. **계약 서류 6종 통합 PDF 생성 및 출력 모달 (`src/components/ContractDocumentBundleModal.tsx`)**:
   - 현재 계약 DB(`contracts`)에 등록된 모든 계약 목록을 드롭다운으로 실시간 검색/선택.
   - 선택된 계약의 거래처, 현장, 계약기간, 체결 장비 목록 요약 카드 표출.
   - `[📥 6종 통합 PDF 다운로드]` 및 `[👁️ 새 창 미리보기]` 지원.
   - 각 단계별 실시간 생성 게이지 및 완료 페이지 수 표출.
3. **계약 관리 (`src/pages/Contracts.tsx`) 화면 연동**:
   - 계약 목록 툴바 상단에 `[📑 6종 통합 서류팩 PDF]` 버튼 배치.
   - 계약 상세 뷰의 `[6종 통합 서류팩 PDF]` 버튼 클릭 시 해당 계약이 자동 선택된 상태로 모달 표출.
...
### 🌟 반영 내용
1. **Cloudflare R2 S3 호환 서버리스 API 엔드포인트 구현 (`api/r2.ts`)**:
   - 구글 드라이브의 인증 팝업, 앱스크립트 차단 및 폴더 API 제약을 원천 극복하기 위해 글로벌 오브젝트 스토리지인 Cloudflare R2 연동 API 구축.
   - S3 표준 `ListObjectsV2`를 통해 버킷 내 모든 하위 폴더 계층과 파일 트리를 0.1초 만에 100% 재귀적으로 수집.
2. **R2 ➔ 로컬 PC 무인 미러링 동기화 엔진 탑재 (`src/services/r2MirrorSync.ts`)**:
   - R2 버킷의 모든 서식·증빙 파일들을 로컬 사이드카 에이전트(`C:\KiyeunAgent\drive_mirror\`)에 하위 폴더 구조 그대로 100% 자동 생성 및 무손실 복제.
   - 실시간 동기화 % 진행률 모니터링 토스트 연동.
3. **설정 화면 내 Cloudflare R2 스토리지 패널 및 파일 브라우저 탑재 (`GoogleConfig.tsx`)**:
   - Account ID, Bucket Name, Access Key, Secret Key, Public Domain 설정 폼 및 [R2 연결 검증], [R2 파일 목록], [로컬 에이전트 동기화] 액션 완결.
   - 전사 표준 헌장 준수 (무수식어 건조한 명사·동사, 세로 스택 레이아웃, 줄바꿈 방지).
4. **로컬 에이전트 바이너리 재컴파일 및 배포 동기화**:
   - `/api/get-file` 엔드포인트에 R2 직결 URL 캐시 파이프라인 추가 및 최신 버전(`v1.116.0.Build.233`) 바이너리 컴파일 완료.

---

# Release Notes (v1.115.0.Build.232 - 2026-08-18 00:15)

## 🚫 [구글 OAuth 계정선택 팝업 0회 영구 제거 & 캐시 우선 로컬 API 탑재] 로컬 미러링 파이프라인 완결

### 🌟 반영 내용
1. **구글 계정선택 OAuth 팝업 호출 전면 영구 차단 (`Dashboard.tsx`, `GoogleConfig.tsx`)**:
   - 계약 서류 결합, 엑셀/드라이브 원본 PDF 병합, 테스트 실행 시 발생하던 구글 로그인/계정 선택 팝업을 100% 원천 제거.
   - 팝업 없이 로컬 에이전트 미러링 폴더(`C:\KiyeunAgent\drive_mirror\`)의 원본 파일 및 무토큰 공개 다운로드로 0.01초 만에 즉시 결합 처리.
2. **에이전트 캐시 우선 파일 서빙 API 구현 (`/api/get-file`)**:
   - 로컬 미러링 캐시가 존재하면 즉시 반환하고, 부재 시 무토큰 다운로드 후 로컬 캐싱하여 반환하는 고속 캐시 파이프라인 탑재.
3. **구글 설정 화면 `하위 폴더 재귀` 옵션 탑재 (`GoogleConfig.tsx`)**:
   - 최상위 구글 드라이브 루트 폴더 하위에 존재하는 서브 디렉토리까지 재귀적으로 미러링할 수 있는 옵션 제공 (전사 UI 표준 헌장 준수).
4. **유틸리티 SSOT 정리 및 에이전트 버전 동기화**:
   - 중복 파일 유틸(`ensureDirSync`, `writeBase64ToFile`)을 `src/utils/fileUtil.ts`로 단일화.
   - 기대 에이전트 버전(`EXPECTED_AGENT_VERSION`)을 `v1.115.0.Build.232`로 상향 동기화.

---

# Release Notes (v1.114.0.Build.231 - 2026-08-17 23:45)

## 🌐 [토큰 0회 공개 다이렉트 미러링 엔진 정립] 구글 인증 팝업 0% & 무인 자동 수신 완결

### 🌟 반영 내용
1. **무토큰 다이렉트 공개 다운로드 파이프라인 탑재 (`downloadPublicDriveFile`)**:
   - 공개 공유된 구글 드라이브 파일들을 별도 Google OAuth 토큰이나 로그인 팝업 없이 100% 무인으로 고속 수신.
2. **사용자 PC 구글 드라이브 무설치 환경 완벽 지원**:
   - 직원 PC에 구글 드라이브 데스크톱 앱이 깔려있지 않아도, 웹 브라우저 접속 즉시 `C:\KiyeunAgent\drive_mirror\`로 원본 서식과 증빙들을 실시간 자동 복제.

---

# Release Notes (v1.113.0.Build.230 - 2026-08-17 23:41)

## 🚫 [구글 OAuth 팝업 자동 호출 영구 차단 & 로컬 직결 미러링 탑재] This app is blocked 차단 원천 제거

### 🌟 반영 내용
1. **사용자를 방해하는 자동 구글 로그인 팝업 호출 전면 영구 차단 (`driveMirrorSync.ts`)**:
   - 브라우저 로딩 시 사용자의 의사와 무관하게 구글 OAuth 팝업이 뜨고 `This app is blocked` 에러로 차단되던 백그라운드 OAuth 호출을 100% 원천 제거.
2. **로컬 구글 드라이브 폴더 직결 미러링 API 구현 (`/api/sync-local-path`)**:
   - 구글 클라우드 권한 승인이나 팝업 없이도, 로컬 PC에 존재하는 파일들을 `C:\KiyeunAgent\drive_mirror\`로 0.01초 만에 즉시 무손실 복제할 수 있는 로컬 직결 파이프라인 개설.

---

# Release Notes (v1.112.0.Build.229 - 2026-08-17 23:29)

## 📡 [미러링 실시간 진행상황 시각화 & 플로팅 토스트 탑재] 어떤 파일이 동기화되는지 100% 투명 표출

### 🌟 반영 내용
1. **실시간 미러링 플로팅 토스트 컴포넌트 탑재 (`MirrorSyncProgressToast.tsx`)**:
   - 미러링이 작동할 때 화면 우측 하단에 `[드라이브 로컬 미러링 중... (4/18)]`, `현재 다운로드 중인 파일명`, `실시간 프로그레스 바(%)`를 즉시 시각화.
   - 완료 시 `✅ 구글 드라이브 18개 파일 미러링 완료` 통지와 함께 4초 후 자동 페이드아웃.
2. **헤더 배지 상태 동적 라벨 연동 (`AgentHeaderBadge.tsx`)**:
   - 미러링 작동 중일 때 최상단 배지가 `🔄 미러링 (4/18)` 로 실시간 텍스트 전환되어 현재 시스템 작업 상태를 직관적으로 확인 가능.

---

# Release Notes (v1.111.0.Build.228 - 2026-08-17 23:28)

## 📦 [하위 폴더 트리 자동 생성 에이전트 바이너리 배포] v1.111.0.Build.228 빌드 및 디지털 서명 완료

### 🌟 반영 내용
1. **에이전트 하위 디렉토리 재귀 자동 생성 엔진 탑재 (`agent.js`)**:
   - 구글 드라이브의 서브폴더(`업무용 엑셀등`, `장비 관련 파일들`, `KY_Lift` 등) 경로가 전달될 때 로컬 디스크에 폴더를 자동 생성(`fs.mkdirSync`)하고 버전 아카이빙을 안전하게 처리.
2. **KiyeunAgent.exe 바이너리 빌드 & 디지털 서명 완결**:
   - `public/downloads/KiyeunAgent.exe` 최신 빌드 배포. 더블클릭 시 기존 프로세스 자동 종료 후 스마트 바통 터치.
...
### 🌟 반영 내용
1. **구글 드라이브 하위 폴더 재귀 탐색 엔진 구현 (`listDriveFolderRecursively`)**:
   - 단일 루트 레벨뿐만 아니라 `01.사업자_통장/`, `02.안전점검서식/`, `04.보험증권/` 등 모든 서브 디렉토리를 깊이 탐색하여 하위 파일 일체를 빠짐없이 수신.
2. **로컬 디렉토리 계층 구조 자동 생성 및 아카이빙 (`agent.js`)**:
   - 구글 드라이브의 폴더 트리와 100% 동일하게 `C:\KiyeunAgent\drive_mirror\하위폴더\...` 를 자동 생성하여 완벽히 미러링.

---

# Release Notes (v1.108.0.Build.225 - 2026-08-17 23:18)

## 🚀 [100% 무인 백그라운드 자동 미러링 동기화] 사용자의 수동 조작 0회 & 로컬 풀 세트 완결

### 🌟 반영 내용
1. **무인 백그라운드 자동 동기화 엔진 정립 (전사 헌장 1.1 최우선 개발 사명)**:
   - 사용자가 버튼을 클릭하거나 신경 쓸 필요 없이, 에이전트가 켜져 있으면 웹 페이지 로드 즉시 백그라운드에서 무음(Silent)으로 100% 자동 동기화 실행.
2. **실제 원본 템플릿 파일 풀 세트 동기화 경로 정밀 매핑**:
   - `반입전체크리스트_양식_원본.pdf` (99KB), `안전점검결과서_양식_원본.pdf` (209KB), `임대차계약서_양식_원본.pdf` (137KB), `거래명세서양식.xlsx` (66KB) 등 실제 원본 파일들이 `C:\KiyeunAgent\drive_mirror\`에 완벽하게 복제 안착.

---

# Release Notes (v1.107.0.Build.224 - 2026-08-17 23:12)

## 📁 [구글 드라이브 폴더 전체 및 시스템 내장 서식 일괄 동기화] drive_mirror 폴더 풀 세트 복제 완결

### 🌟 반영 내용
1. **구글 드라이브 폴더 자동 탐색 및 내장 서식 템플릿 일괄 소집 (`driveMirrorSync.ts`)**:
   - 기존 개별 URL뿐만 아니라 구글 드라이브 루트 폴더 내의 모든 원본 파일과 시스템 내장 서식(견적서, 계약서, 점검표, 거래명세서 등)을 일괄 스캔하여 `C:\KiyeunAgent\drive_mirror\`에 완벽하게 복제.
2. **동기화 파일 목록 실시간 확장**:
   - 더미 파일이 아닌 실제 고품질 서식 파일들이 온전한 용량으로 로컬 폴더에 안착되도록 동기화 파이프라인 전면 개편.

---

# Release Notes (v1.106.0.Build.223 - 2026-08-17 23:05)

## 🌐 [구글 드라이브 무인 자동 미러링 & 헤더 팝오버 현황 뷰어 구축] 에이전트 가동 시 실시간 자동 복제 및 원클릭 동기화 탑재

### 🌟 반영 내용
1. **구글 드라이브 실시간 미러링 동기화 엔진 구현 (`driveMirrorSync.ts`)**:
   - 에이전트가 `ONLINE`으로 감지되면 설정에 등록된 7대 원본 서식/증빙 파일(사업자등록증, 통장사본, 계약서 서식, 안전점검 양식 등)을 `C:\KiyeunAgent\drive_mirror\`로 무음 자동 복제.
2. **최상단 헤더 배지 드롭다운에 로컬 미러링 현황 뷰어 탑재 (`AgentHeaderBadge.tsx`)**:
   - `C:\KiyeunAgent\drive_mirror\`에 복제된 파일 목록(이름, 크기)을 실시간으로 표출.
   - **`[⚡ 지금 동기화]`** 버튼을 통해 사용자가 원할 때 언제든지 원클릭으로 최신 구글 드라이브 파일들을 로컬로 일괄 갱신 가능.

---

# Release Notes (v1.105.0.Build.222 - 2026-08-17 22:43)

## 📌 [헤더 배지 간결화 & 약식 버전 표기 적용] 콜사인 생략 및 대시보드 웰컴 바 완전 청정화

### 🌟 반영 내용
1. **대시보드 웰컴 바 배지 및 재시작 버튼 완전 삭제**:
   - 스크린샷 지시 사항에 따라 대시보드 웰컴 바 내부의 에이전트 배지 및 버튼을 100% 제거.
2. **최상단 헤더 배지 표기 텍스트 초간결화 (`AgentHeaderBadge.tsx`)**:
   - **콜사인 완전 생략**으로 가로 길이 대폭 축소.
   - **약식 버전 표기 적용**:
     - 🟢 **최신**: `🟢 에이전트 v1.100`
     - 🟡 **버전 차이**: `🟡 v1.98 ➔ v1.100` (최신과 현재 버전 차이 약식 표기)
     - 🔴 **미실행**: `🔴 에이전트 미실행`
