...
- **불완전 정보 고객 필터링 및 다차원 엑셀 다운로드 기능 추가 (`Customers.tsx`)**:
  - 고객사 목록 상단에 **`⚠️ 불완전 정보 고객만 보기`** 토글 필터를 신설하여, 필수 입력 정보(사업자번호, 대표자, 연락처, 이메일, 본사 주소) 중 `미상` 또는 누락이 있는 고객사들만 신속히 선별할 수 있도록 구축했습니다.
...
- **스마트 출고 요청 화면 UI 개선 (`SmartDispatch.tsx`)**:
  - 기존 페이지 상단에 겉돌던 `초기화` 및 `출고 지시 (자동 생성 및 저장)` 버튼을 편집 폼(2단계 카드)의 하단부로 이동시켜, 1단계의 변환(추출) 버튼과 좌우 수평이 맞고 스크롤 흐름에 따라 순차 입력 후 저장할 수 있도록 레이아웃 편의성을 향상했습니다.
