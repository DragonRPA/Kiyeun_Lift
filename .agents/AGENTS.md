...
- **버전 증가 규칙**:
  1. **X (Major / 첫번째 값)**: 초대형 메이저 플랫폼 개편 시 증가
  2. **Y (Minor / 두번째 값)**: **신규 메뉴 추가 또는 DB 스키마/테이블 증가 시** 1 증가
  3. **Z (Patch / 세번째 값)**: **오류/버그 수정 또는 기존 메뉴/기능 수정 및 개편 시** 1 증가
  4. **Build.N (Build / 네번째 값)**: 기타 큰 의미 없는 라벨 오타 수정, UI 레이아웃 단순 조정 시 5자리 Build 번호 1 증가 (`Build.00001`, `Build.00002`...)
- 모든 Git 커밋 및 `RELEASE_NOTES.md` 작성 시 위 버전 표기 규칙과 배포 날짜/구체적 시간 정보(예: `2026-07-25 17:03`)를 예외 없이 상시 표기합니다.
