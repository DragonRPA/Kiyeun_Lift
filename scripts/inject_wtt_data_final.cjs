...
const validProductModels = [
  ...
  'GS-3246 E-Drive', 'GS-4047', 'GS-4655', 'SKY-800', 'GENIE-1000', 'LIFT-1200'
];
...
users.forEach(u => {
  menuList.forEach(m => {
    let canView = false, canSave = false;
    if (u.role === 'ADMIN') {
      canView = canSave = true;
...
// 2. 외부 거래처 7개 (type: 'RENTAL' 통일)
const vendors = [
  { id: 'VND-0000001', name: '(주)한국렌탈', type: 'RENTAL', bizRegNo: '101-81-12345', types: '["RENTAL"]', representative: '김한국', contact: '02-1588-1111', email: '77.victor.lee@gmail.com', address: '서울시 서초구 강남대로 101', bankAccount: '신한은행 110-123-456789', isActive: true, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z' },
  { id: 'VND-0000002', name: '(주)AJ네트웍스', type: 'RENTAL', bizRegNo: '102-81-23456', types: '["RENTAL"]', representative: '박에이제이', contact: '02-1588-2222', email: '77.victor.lee@gmail.com', address: '서울시 송파구 정의로 8길 9', bankAccount: '우리은행 1002-234-567890', isActive: true, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z' },
  { id: 'VND-0000003', name: '(주)롯데렌탈', type: 'RENTAL', bizRegNo: '103-81-34567', types: '["RENTAL"]', representative: '이롯데', contact: '02-1588-3333', email: '77.victor.lee@gmail.com', address: '서울시 중구 통일로 10', bankAccount: '국민은행 345-12-345678', isActive: true, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z' },
  { id: 'VND-0000004', name: '(주)글로벌리프트', type: 'RENTAL', bizRegNo: '104-81-45678', types: '["RENTAL"]', representative: '최글로벌', contact: '031-1588-4444', email: '77.victor.lee@gmail.com', address: '경기도 평택시 산단로 50', bankAccount: '하나은행 456-91-234567', isActive: true, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z' },
  { id: 'VND-0000005', name: '(주)대한고소장비', type: 'RENTAL', bizRegNo: '105-81-56789', types: '["RENTAL"]', representative: '정도고소', contact: '031-1588-5555', email: '77.victor.lee@gmail.com', address: '경기도 화성시 남양읍 120', bankAccount: '기업은행 567-01-345678', isActive: true, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z' },
  { id: 'VND-0000006', name: '(주)케이렌탈', type: 'RENTAL', bizRegNo: '106-81-67890', types: '["RENTAL"]', representative: '강케이', contact: '032-1588-6666', email: '77.victor.lee@gmail.com', address: '인천시 서구 원창동 77', bankAccount: '신한은행 110-345-678901', isActive: true, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z' },
  { id: 'VND-0000010', name: '(주)기연부품소모품몰', type: 'RENTAL', bizRegNo: '109-81-99999', types: '["PARTS"]', representative: '정부품', contact: '02-1588-9999', email: '77.victor.lee@gmail.com', address: '서울시 구로구 중앙유통단지 다동 101호', bankAccount: '우리은행 1002-999-888777', isActive: true, createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z' }
];
...
validProductModels.forEach((mName) => {
  for (let i = 1; i <= 20; i++) {
    ...
    assets.push({
      ...
      assetNo,
      modelName: mName,
      serialNo: `SN-${mName.replace(/[^A-Za-z0-9]/g, '').slice(0, 4).toUpperCase()}-${acqYear}-${String(assetIdx).padStart(4, '0')}`,
      manufacturer: 'GENIE',
...
months.forEach((ym, mIdx) => {
  ...
  vendors.filter(v => v.id !== 'VND-0000010').forEach((v, vIdx) => {
...
async function upsertBatch(table, rows, batchSize = 100) {
  ...
  console.log(`✅ [${table}] 주입 완료!`);
}

async function main() {
  console.log('=== Supabase WTT 100% 무결점 완결 주입 시작 ===');
  
  // 1단계: 사용자 6명, 권한
  await upsertBatch('users', users);
  await upsertBatch('permissions', permissions);
  
  // 2단계: 거래처 7개, 운송사 6개, 기사 24명
  ...
  // 5단계: 계약 (반드시 sites/customers/users/assets 완료 후)
  ...
  console.log('🎉🎉🎉 === Supabase WTT 100% 무결점 완결 주입 대성공! ===');
}
